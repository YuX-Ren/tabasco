...
from tabasco.utils.ema_utils.ema_callback import EMA, EmaModelCheckpoint
from tabasco.utils.fetch_last_ckpt import fetch_last_ckpt
from tabasco.utils.lora_utils import replace_lora_layers
from tabasco.utils.metric_utils import (
    transform_global_percentage_to_mask_dropout,
)
from tabasco.utils.seed_callback import SeedCallback
from tabasco.utils.training_analysis_utils import (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_name",
        type=str,
        default="training_pldm_wo_pair_200M",
    ...
    datamodule_cfg_path = os.path.join(root, "configs", "datamodule", "lmdb_datamodule.yaml")
    paths_cfg_path = os.path.join(root, "configs", "paths", "default.yaml")
    if os.path.exists(datamodule_cfg_path) and os.path.exists(paths_cfg_path):
        cfg_paths = OmegaConf.load(paths_cfg_path)
        cfg_data = OmegaConf.load(datamodule_cfg_path)
        cfg_root = OmegaConf.create({"paths": cfg_paths, "datamodule": cfg_data})
        if "lmdb_dir" not in cfg_root.datamodule:
            cfg_root.datamodule.lmdb_dir = os.path.join(root, "data", "lmdb_pdb")
        if cfg_root.datamodule.get("_target_") != "tabasco.data.lmdb_datamodule.LmdbDataModule":
            cfg_root.datamodule._target_ = "tabasco.data.lmdb_datamodule.LmdbDataModule"
        datamodule: LightningDataModule = hydra.utils.instantiate(cfg_root.datamodule)
        cfg_data = cfg_root.datamodule
    else:
        cfg_data = OmegaConf.create(
            {
                "data_dir": os.path.join(root, "data", "alphadb.pt"),
                "val_data_dir": os.path.join(root, "data", "scope.pt"),
                "test_data_dir": os.path.join(root, "data", "scope.pt"),
                "lmdb_dir": os.path.join(root, "data", "lmdb_pdb"),
        ...
        datamodule = LmdbDataModule(**cfg_data)
        datamodule.setup()
    # Set logger
    wandb_logger = None
    if cfg_exp.log.log_wandb and not args.nolog:
        wandb_logger = WandbLogger(project=cfg_exp.log.wandb_project, name=run_name, offline=True)
    ...
    # Define model
    model = ProteinLDM(cfg_exp, store_dir=root_run)

    # Load pretrained weights for encoder and decoder
    if cfg_exp.model.get("pretrained_weights") is not None:
        log_info("Loading pretrained encoder and decoder weights")
        model.load_autoencoder_weights(cfg_exp)
