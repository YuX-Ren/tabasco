...
from src.ase_notebook.atom_info import create_info_lines
from src.ase_notebook.atoms_convert import (
    convert_to_atoms,
    deserialize_atoms,
    serialize_atoms,
)
from src.ase_notebook.backend.gui import AtomGui, AtomImages
from src.ase_notebook.backend.svg import (
    create_axes_elements,
    create_svg_document,
    generate_svg_elements,
    svg_to_pil,
)
from src.ase_notebook.backend.threejs import (
    RenderContainer,
    create_world_axes,
    generate_3js_render,
    make_basic_gui,
)
from src.ase_notebook.color import lighten_webcolor
from src.ase_notebook.configuration import ViewConfig
from src.ase_notebook.data import load_data_file
from src.ase_notebook.draw_utils import (
    compute_projection,
    get_rotation_matrix,
...
class AseView:
    ...
    def get_atom_labels(self, atoms):
        ...
        if self.config.atom_label_by == "element":
            if "occupancy" in atoms.info:
                labels = [",".join(atoms.info["occupancy"][t].keys()) for t in atoms.get_tags()]
            else:
    ...
    def _initialise_elements(self, atoms, center_in_uc=False, repeat_uc=(1, 1, 1)):
        ...
        if config.show_uc_repeats:
            atoms.info["unit_cell_repeat"] = (
                repeat_uc if isinstance(config.show_uc_repeats, bool) else config.show_uc_repeats
            )
    ...
    def _add_element_properties(
        ...
        ghost_atoms = (
            atoms.get_array("ghost") if "ghost" in atoms.arrays else [False for _ in atoms]
        )
        ...
        element_groups["atoms"].set_property_many(
            {
                ...
                "label": [
                    None
                    if (g and not config.ghost_show_label) or (not config.atom_show_label)
                    else l
                    for l, g in zip(atom_labels, ghost_atoms)
                ],
                "ghost": ghost_atoms,
                "fill_opacity": [
                    config.ghost_opacity if g else config.atom_opacity for g in ghost_atoms
                ],
                "occupancy": [
        ...
        element_groups["cell_lines"].set_property_many({"color": config.uc_color}, element=False)
        if config.bond_color_by == "atoms":
            element_groups["bond_lines"].set_property(
                "color",
                [
                    (atom_colors[i], atom_colors[j])
                    for i, j in element_groups["bond_lines"].get_elements_property("atom_index")
                ],
                element=True,
            )
            ...
            element_groups[miller_type].set_property_many(
                {
                    "fill_color": [
                        ...
                        for i in element_groups[miller_type].get_elements_property("index")
                    ],
                    "stroke_color": [
    ...
    def make_svg(self, atoms, center_in_uc=False, repeat_uc=(1, 1, 1)):
        ...
        center, scale = compute_projection(element_groups, config.canvas_size, rotation_matrix)
        scale *= config.zoom
    ...
    def make_gui(
        ...
        gui = AtomGui(config=self.config, images=images, element_colors=self.get_element_colors())
        if bring_to_top:
    ...
    def launch_gui_subprocess(self, atoms, center_in_uc=False, repeat_uc=(1, 1, 1), test_init=2):
...
AseView.__init__.__doc__ = "kwargs are used to initialise ViewConfig:" f"\n\n{ViewConfig.__doc__}"
