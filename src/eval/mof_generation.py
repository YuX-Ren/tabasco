        ...
        for s in tqdm(valid_structs, desc="    MOFChecker"):
            try:
                ...
                for k, v in desc.items():
                    if type(v) == bool:
                        if k in ["has_3d_connected_graph", "is_porous"]:
                            continue
                        if k in ["has_carbon", "has_hydrogen", "has_metal"]:
...
def array_dict_to_structure(
    ...
    #     lengths = (100 * np.ones_like(lengths)).squeeze().tolist()
    #     angles = (90 * np.ones_like(angles)).squeeze().tolist()
    #     sample_idx = x["sample_idx"]
