...
from tabasco.nn.feature_factory import FeatureFactory
from tabasco.nn.pair_bias_attn.pair_bias_attn import PairBiasAttention
from tabasco.nn.alphafold3_pytorch_utils.modules import (
    AdaptiveLayerNorm,
    AdaptiveLayerNormOutputScale,
    Transition,
)
from tabasco.nn.alphafold3_pytorch_utils.utils import (
...
class ProteinTransformerAF3(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.latent_add_place = kwargs.get("latent_add_place", "cond")
        self.apply_inv_folding = kwargs.get("apply_inv_folding", False)
        ...
        if self.sampling_ratio > 1:
            num_sampling_blocks = int(math.log(self.sampling_ratio, 2))
            if self.module_type == "encoder":
                ...
                self.latent_down = torch.nn.Sequential(
                    torch.nn.Linear(kwargs["token_dim"], kwargs["dim_latent"], bias=False),
                    torch.nn.LayerNorm(kwargs["dim_latent"], elementwise_affine=False)
                )
            if self.module_type == "decoder":
                self.upsample = lambda x, size: F.interpolate(x, size=size, mode="nearest")
                self.latent_up = torch.nn.Linear(kwargs["dim_latent"], kwargs["dim_cond"], bias=False)
        else:
        ...
        if self.update_pair_repr and self.module_type == "decoder":
    ...
    def _extend_w_registers(self, seqs, pair, mask, cond_seq):
        ...
        elif self.registers.ndim == 3:
            reg_expanded = self.registers  # [b, r, dim_token]
    ...
    def forward(self, batch_nn: Dict[str, torch.Tensor]):
        ...
        if self.module_type == "decoder":
            ...
            if self.dim_latent is not None:
                single_repr = self.latent_up(single_repr) * mask[..., None]
        else:
            single_repr = None
        ...
        input_map = dict( encoder="x_1", decoder="x_t" )
        assert input_map[self.module_type] in batch_nn, \
            f"input {input_map[self.module_type]} is required for vae {self.module_type} "
        coors_3d = batch_nn[input_map[self.module_type]] * coords_mask[..., None]  # [b, n, 3]
        ...
        if single_repr is not None and self.latent_add_place == "seq":
            if self.module_type != "decoder":
                raise ValueError("single_repr as sequence is only supported for decoder")
            seqs = seqs + single_repr
        ...
        # Undo registers
        seqs, pair_rep, mask, registers = self._undo_registers(seqs, pair_rep, mask)
        
        if self.module_type == "encoder":
            # if self.bottleneck_size > 0:
            #     # Downsample seqs to match the bottleneck size
            #     seqs = seqs * mask[..., None]
            #     seqs = F.adaptive_avg_pool1d(seqs.mT, self.bottleneck_size).mT
            if self.sampling_ratio > 1:
                # Downsample seqs given the sample ratio
                seqs = seqs * mask[..., None]
                seqs_down = seqs.mT
                for conv in self.downsample:
                    seqs_down = conv(seqs_down)
                seqs = seqs_down.mT
            if self.dim_latent is not None:
                seqs = self.latent_down(seqs)
        ...
        if self.module_type == "encoder":
            ...
            if not self.ca_only:
                final_coors = self.coors_3d_decoder(seqs, q_skip, c_skip, p_skip, coords_mask, pair_mask) # [b, n, 3]
            else:
...
class ProteinLatentTransformer(torch.nn.Module):
    def __init__(self, **kwargs):
        super(ProteinLatentTransformer, self).__init__()
        self.nlayers = kwargs["nlayers"]
        self.token_dim = kwargs["token_dim"]
        ...
        # self.out_layernorm = torch.nn.LayerNorm(kwargs["token_dim"])
    ...
    def _extend_w_registers(self, seqs, pair, mask, cond_seq):
        ...
        b, n, _ = seqs.shape
        dim_pair = pair.shape[-1] if pair is not None else None
        r = self.num_registers
        dim_cond = cond_seq.shape[-1]
    ...
    def _undo_registers(self, seqs, pair, mask):
        ...
        if pair is not None:
            ...
            return seqs[:, r:, :], None, mask[:, r:], seqs[:, :r, :]
        
    def forward(self, batch_nn: Dict[str, torch.Tensor]):
        
        x_t, mask = batch_nn["x_t"], batch_nn["mask"]
        
        seqs = self.linear_xt(x_t) * mask[..., None]
        ...
        seq_f_repr = self.init_repr_factory(batch_nn)  # [b, n, token_dim]
        seqs = seqs + seq_f_repr  # [b, n, token_dim]
        seqs = seqs * mask[..., None]  # [b, n, token_dim]
        ...
        # Undo registers
        seqs, pair_rep, mask, _ = self._undo_registers(seqs, pair_rep, mask)
        
        # seqs = self.out_layernorm(seqs)
