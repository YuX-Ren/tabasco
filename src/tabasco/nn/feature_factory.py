...
from tabasco.utils.ff_utils.pdb_utils import extract_cath_code_by_level
from tabasco.utils.ff_utils.idx_emb_utils import get_index_embedding, get_time_embedding
from tabasco.utils.ff_utils.pdb_utils import extract_ca
...
class ZeroFeat(Feature):
    ...
    def forward(self, batch):
        ...
        if self.mode == "seq":
            return torch.zeros((b, n, self.dim), device=mask.device)
        elif self.mode == "pair":
            return torch.zeros((b, n, n, self.dim_feats_out), device=mask.device)
        else:
...
class IdxEmbeddingSeqFeat(Feature):
    ...
    def forward(self, batch):
        ...
        if "residue_pdb_idx" in batch:
            ...
            mask = batch["mask"]  # [b, n]
            b, n = mask.shape[0], mask.shape[1]
            inds = torch.Tensor([[i + 1 for i in range(n)] for _ in range(b)]).long().to(
                mask.device
            )  # [b, n]
...
class ChainBreakPerResidueSeqFeat(Feature):
    ...
    def forward(self, batch):
        ...
        if "chain_breaks_per_residue" in batch:
            ...
            mask = batch['mask']  # [b, n]
            b, n = mask.shape[0], mask.shape[1]
            chain_breaks = torch.zeros((b, n), device=mask.device) * 1.0  # [b, n]
...
class XscSeqFeat(Feature):
    ...
    def forward(self, batch):
        if "x_sc" in batch:
            ...
            mask = batch["mask"]  # [b, n]
            b, n = mask.shape[0], mask.shape[1]
            return torch.zeros(b, n, 3, device=mask.device)
        

class XscLatentSeqFeat(Feature):
    """Computes feature from self conditining sequence, seq feature of shape [b, n, token_dim]."""

    def __init__(self, **kwargs):
        super().__init__(dim=kwargs["dim_latent"])

    def forward(self, batch):
        if "x_sc_latent" in batch:
            return batch["x_sc_latent"]  # [b, n, dim_latent]
        else:
            # If we do not provide self-conditioning as input to the nn
            mask = batch["mask"]  # [b, n]
            b, n = mask.shape[0], mask.shape[1]
...
#         self.r_max = 32
#         self.s_max = 2
#         assert seq_sep_dim == 2 * (self.r_max + 1)
...
#             input=inds[:, :, None] - inds[:, None, :] + self.r_max,
#             min=0,
#             max=2 * self.r_max
#         )
...
class XtPairwiseDistancesPairFeat(Feature):
    ...
    def __init__(self, xt_pair_dist_dim, xt_pair_dist_min, xt_pair_dist_max, **kwargs):
        ...
        self.ca_only = kwargs.get("ca_only", False)

    def forward(self, batch):
        if self.ca_only:
            x = batch["x_t"]
        else:
            x = extract_ca(batch["x_t"])
        return bin_pairwise_distances(
            x=x,
            min_dist=self.min_dist,
            max_dist=self.max_dist,
...
class X1PairwiseDistancesPairFeat(Feature):
    """Computes pairwise distances and returns feature of shape [b, n, n, dim_pair_dist]."""

    def __init__(self, x1_pair_dist_dim, x1_pair_dist_min, x1_pair_dist_max, **kwargs):
        super().__init__(dim=x1_pair_dist_dim)
        self.min_dist = x1_pair_dist_min
        self.max_dist = x1_pair_dist_max
        self.ca_only = kwargs.get("ca_only", False)
...
class XscPairwiseDistancesPairFeat(Feature):
    ...
    def forward(self, batch):
        if "x_sc" in batch:
            ...
            mask = batch["mask"]
            b, n = mask.shape[0], mask.shape[1]
            return torch.zeros(b, n, n, self.dim, device=mask.device)
    ...
    def get_creator(self, f, **kwargs):
        ...
        if self.mode == "seq":
            if f == "time_emb":
                ...
                return XscLatentSeqFeat(**kwargs)
            elif f == "motif_x1":
            ...
            elif f == "x1_pair_dists":
                return X1PairwiseDistancesPairFeat(**kwargs)
