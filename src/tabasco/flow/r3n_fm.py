...
from tabasco.utils.align_utils.align_utils import mean_w_mask
...
class R3NFlowMatcher:
    ...
    def full_simulation(
        ...
        if sampling_mode == "vf":
            print(f"Sampling vf: nsteps={nsteps}")
        elif sampling_mode == "sc":
            print(
                f"Sampling sc: nsteps={nsteps}, schedule={schedule_mode}, param={schedule_p}, gt={gt_mode}, gt_p={gt_p}, gt_clamp={gt_clamp_val}, temp={sc_scale_noise}"
            )
            ...
            for step in tqdm(range(nsteps)):
                ...
                if fixed_structure_mask is None:
                    nn_in = {
                        ...
                        "encoder_registers": encoder_registers,
                    }
                else:
                    nn_in = {
                        "x_t": x,
                ...
                if step > 0 and self_cond:
                    # Self-conditioning
                    if single_repr is not None:
                        # ProteinAE
                        if mask.shape[-1] == coords_mask.shape[-1]:
                            nn_in["x_sc"] = x_1_pred
                        else:
                            nn_in["x_sc"] = extract_ca(x_1_pred)
                    else:
                        # ProteinAE-PLDM
