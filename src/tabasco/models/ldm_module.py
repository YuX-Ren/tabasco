...
class LatentDiffusionLitModule(LightningModule):
    ...
    def __init__(
        ...
        self.num_nodes_bincount = {
            ...
            "qmof150": torch.nn.Parameter(
                torch.load(
                    os.path.join(self.hparams.sampling.data_dir, f"qmof_num_nodes_bincount.pt"),
                    map_location="cpu",
                ),
                requires_grad=False,
            ),
    ...
    def forward(self, batch, sample_posterior: bool = True):
        ...
        dataset_idx = batch['data_type'] + 1  # 0 -> null class
                    ...
                    generation_evaluators[dataset].append_pred_array(
                        {
                            ...
                            **({"lattices": _lattices.detach().cpu().numpy()} if dataset == "mp20" or dataset == "qmof150" else {"lattices": None}),
                            **({"frac_coords": _frac_coords.detach().cpu().numpy()} if dataset == "mp20" or dataset == "qmof150" else {"frac_coords": None}),
