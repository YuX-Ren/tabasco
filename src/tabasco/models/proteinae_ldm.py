...
from tabasco.flow.r3n_fm import R3NFlowMatcher
from tabasco.nn.protein_transformer import ProteinTransformerAF3, ProteinLatentTransformer
from tabasco.models.model_trainer_base import ModelTrainerBase, _extract_cath_code
from tabasco.models.lightning_tabasco import LightningTabasco
from tabasco.utils.align_utils.align_utils import kabsch_align
from tabasco.nn.motif_factory import SingleMotifFactory
from tabasco.utils.ff_utils.pdb_utils import mask_cath_code_by_level
from tabasco.chem.constants import ATOM_NAMES_OFFSET
...
class ProteinLDM(ModelTrainerBase):
    ...
    def __init__(self, cfg_exp, store_dir=None):
        ...
        # Neural network (external autoencoder only mode)
        self.encoder = None
        self.decoder = None
        ...
        self.use_external_autoencoder = False
        self.autoencoder_latent_key = cfg_exp.model.get("autoencoder_latent_key", None)
        self.autoencoder_use_ema = cfg_exp.model.get("autoencoder_use_ema", True)
        self.use_autoencoder_decode = cfg_exp.model.get("use_autoencoder_decode", True)
        if cfg_exp.model.get("autoencoder_ckpt"):
            self._load_external_autoencoder(cfg_exp.model.autoencoder_ckpt)
        if not self.use_external_autoencoder:
            raise ValueError(
                "External autoencoder is required in this mode. "
                "Set `model.autoencoder_ckpt` in the config."
            )
        if not self.use_autoencoder_decode:
            raise ValueError(
                "Internal decoder is disabled. Set `model.use_autoencoder_decode: true`."
            )
    ...
    def load_autoencoder_weights(self, cfg_exp):
        ...
        if self.use_external_autoencoder:
            logger.info("External autoencoder is set; skipping internal encoder/decoder loading.")
            return
        if self.encoder is None or self.decoder is None:
            logger.warning("Internal encoder/decoder not initialized; skipping weight loading.")
            return
    ...
    def _apply_ema_weights_to_model(self, model: torch.nn.Module, ckpt_path: str) -> bool:
        """Apply EMA weights if present in a Lightning checkpoint."""
        checkpoint = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        opt_states = checkpoint.get("optimizer_states", [])
        if not opt_states:
            return False
        ema_params_list = opt_states[0].get("ema", None)
        if ema_params_list is None:
            return False
        model_params = list(model.parameters())
        if len(model_params) != len(ema_params_list):
            logger.warning(
                f"EMA param mismatch: model has {len(model_params)} params, EMA has {len(ema_params_list)}."
            )
            return False
        with torch.no_grad():
    ...
    def _load_external_autoencoder(self, ckpt_path: str):
        """Load and freeze external autoencoder from a Lightning checkpoint."""
        logger.info(f"Loading external autoencoder from {ckpt_path}")
        lightning_module = LightningTabasco.load_from_checkpoint(ckpt_path, weights_only=False)
        self.autoencoder = lightning_module.model
        if self.autoencoder_use_ema:
            applied = self._apply_ema_weights_to_model(self.autoencoder, ckpt_path)
            if not applied:
                logger.warning("EMA weights not found; using checkpoint weights for autoencoder.")
        self.autoencoder.requires_grad_(False)
        self.autoencoder.eval()
        self.use_external_autoencoder = True

    def _extract_latent_from_autoencoder(self, encoded):
        """Normalize external autoencoder outputs into a latent tensor."""
        if torch.is_tensor(encoded):
            return encoded
        if hasattr(encoded, "get"):
            candidate_keys = []
            if self.autoencoder_latent_key:
                candidate_keys.append(self.autoencoder_latent_key)
            candidate_keys.extend(["single_repr", "x", "latent", "z", "h"])
        ...
        raise KeyError(
            "Unsupported autoencoder output format; expected tensor or dict with latent key."
        )

    def _encode_latent(self, batch):
        """Encode batch to latent space using external or internal encoder."""
        if self.use_external_autoencoder:
            encoded = self.autoencoder.encode(batch)
            return self._extract_latent_from_autoencoder(encoded)
        if self.encoder is None:
            raise RuntimeError("No external autoencoder set and internal encoder is disabled.")
        return self.encoder(batch).get("single_repr", None)

    def _calculate_model_parameters(self):
        """Calculate total number of trainable parameters."""
        return (
            sum(p.numel() for p in (self.encoder.parameters() if self.encoder else []) if p.requires_grad) +
            sum(p.numel() for p in (self.decoder.parameters() if self.decoder else []) if p.requires_grad) +
            sum(p.numel() for p in self.ldm.parameters() if p.requires_grad)
        )
    
    def predict_clean(self, batch: Dict):
        if self.decoder is None:
            raise RuntimeError("Internal decoder is disabled. Enable external autoencoder decoding.")
    ...
    def _extract_coordinates_and_masks(self, batch):
        ...
        return self._extract_all_atom_coordinates(batch)
    ...
    def _extract_ca_coordinates(self, batch):
        ...
        if "x_1" in batch:
            ...
            x_1 = batch["coords"]
        elif "coords_ca" in batch:
            x_1 = batch["coords_ca"]
        else:
            raise KeyError("Expected 'x_1', 'coords', or 'coords_ca' in batch.")
        ...
        if "mask" in batch:
            ...
            mask = torch.ones(x_1.shape[:-1], device=x_1.device, dtype=torch.bool)

        coords_mask = batch.get("coords_mask", mask)
        return x_1, mask.bool(), coords_mask.bool()

    def _extract_all_atom_coordinates(self, batch):
        """Extract full-atom coordinates and masks (expects atom-level coords)."""
        if "x_1" in batch:
            x_1 = batch["x_1"]
        elif "coords" in batch:
            x_1 = batch["coords"]
        else:
            raise KeyError("Expected 'x_1' or 'coords' in batch for all-atom mode.")
    ...
    def _apply_rotation_augmentation(self, x_1, coords_mask, mask):
        """Apply random rotations, optionally expanding batch size."""
        naug = max(int(self.cfg_exp.model.augmentation.get("naug_rot", 1)), 1)
        if naug > 1:
            x_1 = x_1.repeat(naug, 1, 1)
            coords_mask = coords_mask.repeat(naug, 1)
            mask = mask.repeat(naug, 1)

        rotations = sample_uniform_rotation(
            shape=x_1.shape[:-2], dtype=x_1.dtype, device=x_1.device
        )
        x_1 = torch.matmul(x_1, rotations)
    ...
    def _prepare_batch_data_latent(self, batch):
        ...
        if self.apply_inv_folding and "residue_type" not in batch and "atomics" in batch:
            residue_type = batch["atomics"].argmax(dim=-1) - ATOM_NAMES_OFFSET
            batch["residue_type"] = torch.clamp(residue_type, min=-1)
        batch.update({
            "x_1": x_1,
            "mask": mask,
            "coords_mask": coords_mask,
        })
        if hasattr(batch, "set_non_tensor"):
            batch.set_non_tensor("batch_shape", batch_shape)
            batch.set_non_tensor("n", n)
            batch.set_non_tensor("dtype", dtype)
        else:
    ...
    def _encode_and_prepare_flow_matching_latent(self, batch):
        ...
        # Encode
        with torch.no_grad():
            single_repr = self._encode_latent(batch)
            assert single_repr is not None, "Single repr is None"
        single_repr = self.detach_gradients(single_repr)
        batch["single_repr"] = single_repr
        if single_repr.shape[-1] != self.ae_token_dim:
            raise ValueError(
                f"Latent dim mismatch: expected {self.ae_token_dim}, got {single_repr.shape[-1]}"
            )
    ...
    def _apply_fold_conditional_training(self, batch):
        ...
        if self.cfg_exp.training.fold_cond:
            ...
            cath_code_list = batch.get("cath_code", None)
            if cath_code_list is None:
                return
            for i in range(bs):
    ...
    def _decode_with_autoencoder(self, single_repr: Tensor, mask: Tensor):
        """Decode latent representation with external autoencoder if available."""
        if not self.use_external_autoencoder:
            raise RuntimeError("External autoencoder is not loaded.")
        z = {"x": single_repr}
        padding_mask = ~mask
        return self.autoencoder.decode(z, padding_mask=padding_mask)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        use_autoencoder_decode = self.use_external_autoencoder and self.use_autoencoder_decode
        if self.inf_cfg.inv_folding and not use_autoencoder_decode:
            pred_residue_type = self.decoder.decode_residue_type(single_repr)

        if use_autoencoder_decode:
            decoded = self._decode_with_autoencoder(single_repr, mask)
            return {
                "id": batch.get("id", None),
                "pred_coords": decoded["coords"],
                "pred_atomics": decoded.get("atomics", None),
