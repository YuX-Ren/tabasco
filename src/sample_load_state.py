...
def compute_volume(batch_lattice):
    ...
    return torch.abs(torch.einsum('i,i->', vector_a,
                                  torch.cross(vector_b, vector_c)))
...
# def main():
    ...
    for batch in tqdm.tqdm(datamodule.test_dataloader()):
        ...
        per_batch_rmsd = compute_rmsd_with_kabsch(batch, out_batch, materials_match = datamodule_cfg["train_materials"])
